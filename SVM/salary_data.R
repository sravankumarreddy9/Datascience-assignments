...
class(test_sal)
#graphs to visualize with respect to salary for each variable
...
ggplot(data=train_sal,aes(x=train_sal$Salary, y = train_sal$hoursperweek, fill = train_sal$Salary)) +
  geom_boxplot() +
  ggtitle("Box Plot")
#checking for density plots how the data has been distributed with respect to salary for all variables
...
#creating a model with train dataset with output variable as salary with all variables
model1<-ksvm(train_sal$Salary~., data= train_sal, kernel = "vanilladot")
model1
#predict values with test dataset and model of train dataset
